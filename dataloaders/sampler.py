...
class data_sampler(object):
    ...
    def _read_data(self, file):
        ...
        '''
        if self.args.noise_rate ==0:
            train_dataset = self._read_per_data(file.replace('.json','_train.json'))
        else:
            train_dataset = self._read_per_data(file.replace('.json','_train_noise_{}.json'.format(self.args.noise_rate)))
        if "tacred" not in file:
            val_dataset = self._read_per_data(file.replace('.json','_val.json'))
        else:
            val_dataset = [[] for i in range(self.args.num_of_relation)]
        test_dataset = self._read_per_data(file.replace('.json','_test.json'))          
        return train_dataset, val_dataset, test_dataset
        
    def _read_per_data(self, file):
        data = json.load(open(file, 'r', encoding='utf-8'))
        dataset = [[] for i in range(self.args.num_of_relation)]
        for relation in data.keys():
            ...
            for i, sample in enumerate(rel_samples):
                ...
                if "train" in file:
                    if self.args.noise_rate ==0:
                        tokenized_sample['ori_relation'] = self.rel2id[sample['relation']]
                    else:
                        tokenized_sample['ori_relation'] = self.rel2id[sample['ori_relation']]
                dataset[self.rel2id[relation]].append(tokenized_sample)
        return dataset
